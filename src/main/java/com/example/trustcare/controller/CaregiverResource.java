...
public class CaregiverResource {
    ...
    @POST
    @Path("/{caregiverId}/upload-cnic")
    public Response uploadCNIC(@PathParam("caregiverId") int caregiverId,
                               @FormDataParam("file") InputStream fileInputStream,
                               @FormDataParam("file") FormDataContentDisposition fileMetaData) {
        try {
            // Save file to temp location
            String tempFilePath = "uploads/" + fileMetaData.getFileName();
            Files.copy(fileInputStream, Paths.get(tempFilePath), StandardCopyOption.REPLACE_EXISTING);
            caregiverDAO.uploadCNIC(caregiverId, tempFilePath);

            return Response.ok("CNIC uploaded successfully").build();
        } catch (Exception e) {
            return Response.status(Response.Status.INTERNAL_SERVER_ERROR)
                    .entity("Error uploading CNIC: " + e.getMessage()).build();
        }
